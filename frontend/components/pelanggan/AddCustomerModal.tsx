...
export function AddCustomerModal({ isOpen, onClose }: AddCustomerModalProps) {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: FormValues) => {
      return await customersApi.createCustomer({
        name: data.name,
    ...
    onSuccess: (_, variables) => {
      ...
      toast.success(`Pelanggan "${variables.name}" berhasil ditambahkan!`);
      form.reset();
      setErrorMessage("");
      onClose();
    },
    onError: (error: any) => {
      const msg =
        ...
        error.message ||
        "Gagal menyimpan data pelanggan ke backend.";
      setErrorMessage(msg);
      ...
      <DialogContent className="sm:max-w-[500px] bg-white dark:bg-slate-900 rounded-xl shadow-modal border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100">
        <DialogHeader>
          ...
          <DialogDescription className="text-sm text-slate-500 dark:text-slate-400">
            Masukkan informasi detail kontak pelanggan ke dalam basis data.
          </DialogDescription>
        </DialogHeader>

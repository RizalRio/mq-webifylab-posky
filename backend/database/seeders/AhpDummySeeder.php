...
use App\Models\Tenant; // Pastikan model Tenant kamu ada, atau sesuaikan dengan tabel user/tenant
...
class AhpDummySeeder extends Seeder
{
    ...
    {
        // Ambil Tenant ID pertama sebagai sampel. 
        // (Sesuaikan logika ini jika kamu mengambil tenant_id dari tabel users)
        $tenantId = DB::table('users')->value('tenant_id');

        if (!$tenantId) {
            $this->command->error("Tidak ditemukan tenant_id di database. Buat user/tenant terlebih dahulu.");
            return;
        }

        $this->command->info("Menyiapkan skenario AHP untuk Tenant: {$tenantId}");
        ...
        $kriteria = [
            ...
            'pengiriman' => Criterion::create(['id' => Str::uuid(), 'tenant_id' => $tenantId, 'code' => 'C3', 'name' => 'Waktu Pengiriman', 'type' => 'cost']), // Cost karena makin cepat (sedikit harinya) makin baik
        ];

        // 2. Buat Matriks Perbandingan (Skala 1-9)
        // Skenario: 
        // - Harga (C1) sedikit lebih penting dari Kualitas (C2) -> Nilai 3
        // - Harga (C1) sangat lebih penting dari Pengiriman (C3) -> Nilai 5
        // - Kualitas (C2) sedikit lebih penting dari Pengiriman (C3) -> Nilai 2
        $comparisons = [
        ...
        $suppliers = [
            'A' => Supplier::create(['id' => Str::uuid(), 'tenant_id' => $tenantId, 'name' => 'PT Alfa Makmur', 'contact_person' => 'Andi']),
            'B' => Supplier::create(['id' => Str::uuid(), 'tenant_id' => $tenantId, 'name' => 'CV Beta Logistik', 'contact_person' => 'Budi']),
            'C' => Supplier::create(['id' => Str::uuid(), 'tenant_id' => $tenantId, 'name' => 'UD Gamma Grosir', 'contact_person' => 'Citra']),
        ];

        // 4. Input Nilai Mentah Evaluasi
        $evaluations = [
            // Supplier A: Harga Sedang, Kualitas Sedang, Pengiriman Lama (4 hari)
            ...
            // Supplier B: Harga Mahal, Kualitas Sangat Bagus, Pengiriman Cepat (1 hari)
            ...
            // Supplier C: Harga Sangat Murah, Kualitas Rendah, Pengiriman Sedang (2 hari)
        ...
        $this->command->info("✅ Seeder AHP berhasil dijalankan!");
    }
}
